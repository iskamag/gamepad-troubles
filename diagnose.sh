...
sdlbin="$(dirname "$(readlink -f "$0")")/sdl2-gamepad-test"
if [ -x "$sdlbin" ]; then
    info "running sdl2-gamepad-test (2s probe)..."
    if "$sdlbin" </dev/null >/tmp/.gp_sdl.txt 2>&1 & pid=$!; then
        sleep 2; kill "$pid" 2>/dev/null
    fi
    if grep -q "Joysticks found: [1-9]" /tmp/.gp_sdl.txt; then
        ok "SDL sees joystick(s):"
        grep -E 'Name:|GUID:|Gamepad:' /tmp/.gp_sdl.txt | sed 's/^/      /'
    else
        bad "SDL sees 0 joysticks (but kernel may see it -> SDL mapping/driver issue)"
    fi
    rm -f /tmp/.gp_sdl.txt
else
    warn "sdl2-gamepad-test not built; skipping SDL probe"
    info "build it: gcc -o sdl2-gamepad-test sdl2-gamepad-test.c \$(pkg-config --cflags --libs sdl2)"
fi
